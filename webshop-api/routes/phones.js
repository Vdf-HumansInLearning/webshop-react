        ...
        fs.writeFile(
          ...
          function (err) {
            if (err) {
              ...
              res
                .status(200)
                .send({ message: `Adding phone ${req.body.name}` });
            }
          }
        );
      }
      ...
      fs.writeFile(
        ...
        function (err) {
          if (err) {
            ...
            res
              .status(200)
              .send({ message: `Updating phone ${req.body.name}` });
          }
        }
      );
    } else {
...
function validateProduct(product) {
  let regexProductName =
    /(^[A-Za-z0-9]{1,16})([ ]{0,1})([A-Za-z0-9]{1,16})?([ ]{0,1})?([A-Za-z0-9]{1,16})/;
